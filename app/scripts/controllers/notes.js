angular.module('notes')
  .controller('NotesCtrl', ['$scope', 'ApiRestangular', '_', function($scope, ApiRestangular, _) {
    ...
    $scope.updateNote = function() {
      ...
      $scope.selected.put(null, {
        "If-Match": $scope.selected._etag.$oid
      }).then(function(res) {
    ...
    $scope.deleteNote = function() {
      ApiRestangular.one("notes", $scope.selected._id.$oid).remove(null, {
        "If-Match": $scope.selected._etag.$oid
      }).then(function() {
        $scope.loadNotes();
        $scope.selected = null;
...
function patchNote($scope, $log, toaster, note, data, retryOnConflict, noToasterOnSuccess) {
  if (angular.isUndefined(noToasterOnSuccess)) {
    noToasterOnSuccess = false;
  }

  note.patch(data).then(function() {
    // refresh data (including etag)
    $scope.loadNotes($scope.tableState);

    if (angular.isDefined(toaster) && toaster !== null && !noToasterOnSuccess)
      toaster.pop('success', "Yeah", "Note updated.");
  }, function(response) {
    // reset data
    var updatedNotePromise = note.get();

    $scope.loadNotes($scope.tableState);

    if (response.status === 412) {
      $log.warn("ETag does not match (precondition failed 412) on patching note " + note._id + ", retring " +
        retryOnConflict + " times");
    }

    if (response.status === 412 && angular.isNumber(retryOnConflict) && retryOnConflict > 0) {
      updatedNotePromise.then(function(updatedNote) {
        patchNote($scope, $log, toaster, updatedNote, data, retryOnConflict - 1, noToasterOnSuccess);
      });
      ...
      $log.error("error patching note " + note._id + "; data:" + JSON.stringify(data) + "; response: " + JSON.stringify(
        response));
    }
  });
}
